...
class clsTacxTrainer:
    ...
    def __init__(self, clv, Message):
        if debug.on(debug.Function):
            logfile.Write("clsTacxTrainer.__init__()")
        self.clv = clv
    ...
    def GetTrainer(clv, AntDevice=None):
        ...
        if clv.SimulateTrainer:
            return clsSimulatedTrainer(clv)

        if clv.Tacx_Vortex:
            return clsTacxAntVortexTrainer(clv, AntDevice)
        if clv.Tacx_Genius:
            return clsTacxAntGeniusTrainer(clv, AntDevice)
        if clv.Tacx_Bushido:
        ...
        for hu in [hu1902, hu1902_nfw, hu1904, hu1932, hu1942, hue6be_nfw]:
            try:
                ...
                dev = usb.core.find(
                    idVendor=vendor, idProduct=hu
                )  # find trainer USB device
                if dev:
                    msg = (
                        "Connected to Tacx Trainer T" + hex(hu)[2:]
                    )  # remove 0x from result
        ...
        if not dev:
            ...
            if hu in (hu1902, hu1902_nfw):
                LegacyProtocol = True
                logfile.Console(
                    "Initialising head unit T1902 (iMagic), please wait 5 seconds"
                )
            ...
            if hu == hue6be_nfw:
                logfile.Console(
                    "Initialising head unit T1942 (Fortius), please wait 5 seconds"
                )
            ...
            if dev != False and LegacyProtocol == False:
                ...
                if debug.on(debug.Data2):
                    logfile.Write(
                        "InitialiseTrainer data=%s (len=%s)"
    ...
    def SetPower(self, Power):
        if debug.on(debug.Function):
            logfile.Write("SetPower(%s)" % Power)
    ...
    def SetGrade(self, Grade):
        if debug.on(debug.Function):
            logfile.Write("SetGrade(%s)" % Grade)
        if Grade < -30 or Grade > 30:
            logfile.Console("Grade limitted to range -30 ... 30")
        if Grade > 30:
            Grade = 30
        if Grade < -30:
            Grade = -30
    ...
    def SetWind(self, WindResistance, WindSpeed, DraftingFactor):
        if debug.on(debug.Function):
            logfile.Write(
                "SetWind(%s[def=0.51], %s[def=0], %s[def=1])"
    ...
    def _Grade2Power(self):  # Grade2Power#
        ...
        if debug.on(debug.Function):
            logfile.Write(
                "Grade2Power (TargetGrade=%4.1f%%, Speed=%4.1f, Weight=%3.0f, rR=%s, wR=%s, wS=%s, d=%s) = TargetPower=%3.0fW"
                % (
                    self.TargetGrade,
                    self.VirtualSpeedKmh,
    ...
    def DisplayStateTable(self, FortiusAntState):
        ...
        c1 = (
            constants.WHITE
            if FortiusAntState == constants.faTrainer
            else constants.GREY
        )
        c2 = (
            constants.WHITE
            if FortiusAntState == constants.faWait2Calibrate
            else constants.GREY
        )
        c3 = (
            constants.WHITE
            if FortiusAntState == constants.faCalibrating
            else constants.GREY
        )
        c4 = (
            constants.WHITE
            if FortiusAntState == constants.faActivate
            else constants.GREY
        )
        c5 = (
            constants.WHITE
        ...
        c6 = (
            constants.WHITE
            if FortiusAntState == constants.faStopped
            else constants.GREY
        )
        c7 = (
            constants.WHITE
            if FortiusAntState == constants.faDeactivated
            else constants.GREY
        )
        c8 = (
            constants.WHITE
...
class clsSimulatedTrainer(clsTacxTrainer):
    def __init__(self, clv):
        ...
        if debug.on(debug.Function):
            logfile.Write("clsSimulatedTrainer.__init__()")
        self.OK = True
    ...
    def Refresh(self, _QuarterSecond=None, _TacxMode=None):
        ...
        if False or self.TargetPower < 10:  # Return fixed value
            ...
            if deltaPower < 8:
                ...
                self.CurrentPower = (
                    self.CurrentPower + deltaPower / 8
                )  # Step towards TargetPower
            self.CurrentPower *= 1 + random.randint(-3, 3) / 100  # Variation of 5%

            self.Cadence = (
                100 - min(10, deltaPower) / 10
            )  # Cadence drops when Target increases
            self.Cadence *= 1 + random.randint(-2, 2) / 100  # Variation of 2%

            self.SpeedKmh = (
                35 * self.Cadence / 100
            )  # Speed is 35 kmh at cadence 100 (My highest gear)

            self.HeartRate = HRmax * (
                0.5 + ((self.CurrentPower - 100) / (ftp - 100)) * 0.3
            )
            # As if power is linear with power
            # 100W is reached at 50%
            # ftp  is reached at 80%
            # Assume lineair between 100W and ftp
            if self.HeartRate < HRmax * 0.5:
                self.HeartRate = HRmax * 0.5  # minimize HR
            if self.HeartRate > HRmax:
                self.HeartRate = HRmax  # maximize HR
...
class clsTacxAntVortexTrainer(clsTacxTrainer):
    def __init__(self, clv, AntDevice):
        ...
        if debug.on(debug.Function):
            logfile.Write("clsTacxAntVortexTrainer.__init__()")
    ...
    def _ReceiveFromTrainer(self):
        ...
        # ----------------------------------------------------------------------
        if self.__VortexButtons == ant.VHU_Button_None:
            self.Buttons = 0
        elif self.__VortexButtons == ant.VHU_Button_Left:
            self.Buttons = CancelButton
        elif self.__VortexButtons == ant.VHU_Button_Up:
            self.Buttons = UpButton
        elif self.__VortexButtons == ant.VHU_Button_Enter:
            self.Buttons = OKButton
        elif self.__VortexButtons == ant.VHU_Button_Down:
            self.Buttons = DownButton
        elif self.__VortexButtons == ant.VHU_Button_Right:
            self.Buttons = EnterButton
        ...
        if QuarterSecond:
            ...
            if TacxMode == modeResistance:
                ...
                if self.__AntVHUpaired:
                    ...
                    if debug.on(debug.Function):
                        logfile.Write(
                            "Vortex HU page 172/0x03 (OUT)  Mode=%d" % ant.VHU_Normal
                        )
            ...
            if id == ant.msgID_BroadcastData:
                ...
                if not self.__AntVTXpaired:
                    msg = ant.msg4D_RequestMessage(
                        ant.channel_VTX_s, ant.msgID_ChannelID
                    )
                    messages.append(msg)
                ...
                if DataPageNumber == 0:
                    ...
                    if debug.on(debug.Function):
                        logfile.Write(
                            "Vortex Page=%d (IN) UsingVirtualSpeed=%d Power=%d Speed=%d CalState=%d Cadence=%d"
                            % (
                                DataPageNumber,
                                UsingVirtualSpeed,
                                self.__CurrentPower,
                ...
                # ---------------------------------------------------------------
                    ...
                    (
                        ...
                        Year,
                        DeviceNumber,
                        self.__VTX_AlarmStatus,
                    ) = ant.msgUnpage01_TacxVortexDataSerial(info)
                    if debug.on(debug.Function):
                        logfile.Write(
                            "Vortex Page=%d (IN) DeviceType=%d Year=%d DeviceNumber=%d Alarm=%d"
                            % (
                                DataPageNumber,
                                DeviceType,
                                Year,
                ...
                # ---------------------------------------------------------------
                    ...
                    if debug.on(debug.Function):
                        logfile.Write(
                            "Vortex Page=%d (IN) MajorVersion=%d MinorVersion=%d Build=%d"
                ...
                # ---------------------------------------------------------------
                    ...
                    if debug.on(debug.Function):
                        logfile.Write(
                            "Vortex Page=%d (IN) CalValue=%d VortexID=%d"
        ...
        # -----------------------------------------------------------------------
            ...
            if id == ant.msgID_AcknowledgedData:
                ...
                if DataPageNumber == 221:
                    self.__VortexButtons = ant.msgUnpage221_TacxVortexHU_ButtonPressed(
                        info
                    )
                    if debug.on(debug.Function):
                        logfile.Write(
                            "Vortex HU Page=%d (IN) Buttons=%d"
            ...
            # -------------------------------------------------------------------
                ...
                if not self.__AntVHUpaired:
                    msg = ant.msg4D_RequestMessage(
                        ant.channel_VHU_s, ant.msgID_ChannelID
                    )
                    messages.append(msg)
                ...
                if DataPageNumber == 173 and SubPageNumber == 0x01:  # 0xad:
                    ...
                    if debug.on(debug.Function):
                        logfile.Write(
                            "Vortex HU Page=%d/%#x (IN) Mode=%d Year=%d DeviceType=%d DeviceNumber=%d"
                            % (
                                DataPageNumber,
                                SubPageNumber,
                                self.__VHUmode,
            ...
            if TacxMode == modeResistance:
                if self.TargetMode == mode_Grade:
                    ...
                    if self._CommandCounter < WindResistanceInterval:
                        ...
                        effectiveGrade = (
                            self.TargetGrade + self.__RollingResistance2Grade()
                        )

                        info = ant.msgPage220_01_TacxGeniusSetTarget(
                            self.Channel,
                            ant.GNS_Mode_Slope,
                            effectiveGrade,
                            self.UserAndBikeWeight,
                        ...
                        # ---------------------------------------------------------------
                        WindResistance, WindSpeed = self._ConvertWind(
                            self.WindResistance, self.WindSpeed, self.DraftingFactor
                        )
                        info = ant.msgPage220_02_TacxGeniusWindResistance(
                            self.Channel, WindResistance, WindSpeed
                        )
                        if debug.on(debug.Function):
                            logfile.Write(
        ...
        if Channel == self.Channel:
            ...
            if id == ant.msgID_BroadcastData:
                ...
                if DataPageNumber == 221 and SubPageNumber == 0x01:
                    ...
                    if debug.on(debug.Function):
                        logfile.Write(
                            "Tacx Page=%d/%#x (IN)  Power=%d Speed=%d Cadence=%d Balance=%d"
                            % (
                                DataPageNumber,
                                SubPageNumber,
                                self._CurrentPower,
                ...
                # -----------------------------------------------------------------
                    ...
                    if debug.on(debug.Function):
                        logfile.Write(
                            "Tacx Page=%d/%#x (IN)  Distance=%d Heartrate=%d"
...
class clsTacxAntGeniusTrainer(clsTacxAntTrainer):
    ...
    def _ReceiveFromTrainer(self):
        ...
        if self.__State == GeniusState.CalibrationStarted:
            ...
            self.Message = (
                "* * * * C A L I B R A T I N G  ( D O  N O T  P E D A L ! ) * * * *"
            )
    ...
    def _ConvertWind(self, WindResistance, WindSpeed, DraftingFactor):
        return 0.5 * WindResistance * DraftingFactor * 1000, -250 * WindSpeed / 3.6
    ...
    def SendToTrainer(self, QuarterSecond, TacxMode):
        ...
        if self.__State == GeniusState.Running:
            ...
            self.__State
            in [GeniusState.CalibrationStarted, GeniusState.CalibrationRunning]
            and QuarterSecond
        ):
            RequestInterval = 4
            self._CommandCounter += 1
            ...
            self.__State == GeniusState.CalibrationDone
            or self.__State == GeniusState.CalibrationFailed
        ):
        ...
        if Channel == self.Channel:
            ...
            if id == ant.msgID_BroadcastData:
                ...
                if DataPageNumber == 221 and SubPageNumber == 0x04:
                    ...
                    if debug.on(debug.Function):
                        logfile.Write(
                            "Genius Page=%d/%#x (IN)  CalibrationState=%d CalibrationValue=%d"
                            % (
                                DataPageNumber,
                                SubPageNumber,
                                calibrationState,
...
class clsTacxAntBushidoTrainer(clsTacxAntTrainer):
    ...
    def _ReceiveFromTrainer(self):
        ...
        if self._DeviceNumber:
            ...
            if self._AlarmStatus & ant.BHU_Alarm_Overvoltage:
                msg += " OVERVOLTAGE!"
            if (
                self._AlarmStatus & ant.BHU_Alarm_Overcurrent_1
                or self._AlarmStatus & ant.BHU_Alarm_Overcurrent_2
            ):
                msg += " OVERCURRENT!"
        ...
        if Keycode == ant.VHU_Button_None:
            self.Buttons = 0
        elif Keycode == ant.VHU_Button_Left:
            self.Buttons = CancelButton
        elif Keycode == ant.VHU_Button_Up:
            self.Buttons = UpButton
        elif Keycode == ant.VHU_Button_Enter:
            self.Buttons = OKButton
        elif Keycode == ant.VHU_Button_Down:
            self.Buttons = DownButton
        elif Keycode == ant.VHU_Button_Right:
            self.Buttons = EnterButton
        self.__Buttons = ant.VHU_Button_None
    ...
    def _ConvertWind(self, WindResistance, WindSpeed, DraftingFactor):
        return 0.5 * WindResistance * DraftingFactor * 1000, WindSpeed / 3.6
...
class clsTacxUsbTrainer(clsTacxTrainer):
    ...
    def USB_Read(self):
        ...
        try:
            ...
            if "timeout error" in str(e) or "timed out" in str(
                e
            ):  # trainer did not return any data
                pass
            else:
        ...
        if debug.on(debug.Data2):
            logfile.Write(
                "Trainer recv hdr=%s data=%s (len=%s)"
    ...
    def USB_Read_retry4x40(self, expectedHeader=USB_ControlResponse):
        ...
        if len(data) < 40:
            ...
            logfile.Console(
                "Tacx head unit returns insufficient data, len=%s" % len(data)
            )
            if self.clv.PedalStrokeAnalysis:
                logfile.Console("To resolve, try to run without Pedal Stroke Analysis.")
            else:
                logfile.Console(
                    "To resolve, check all (signal AND power) cabling for loose contacts."
                )
            ...
            self.tacxEvent = False
            logfile.Console(
                "Tacx head unit returns incorrect header %s (expected: %s)"
    ...
    def SendToTrainer(self, _QuarterSecond, TacxMode):
        ...
        if debug.on(debug.Function):
            logfile.Write(
                "clsTacxUsbTrainer.SendToTrainer(T=%s, M=%s, P=%s, G=%s, R=%s, W=%s, PE=%s, S=%s, C=%s)"
                % (
                    TacxMode,
                    self.TargetMode,
                    self.TargetPower,
        ...
        if error:
            ...
            if TacxMode == modeMotorBrake:
                ...
                Target = int(Target)
                data = self.SendToTrainerUSBData(
            ...
            if data != False:
                if debug.on(debug.Data2):
                    logfile.Write(
                        "Trainer send data=%s (len=%s)"
                        % (logfile.HexSpace(data), len(data))
                    )
                    logfile.Write(
                        "                  tacx mode=%s target=%s pe=%s weight=%s cal=%s"
...
class clsTacxLegacyUsbTrainer(clsTacxUsbTrainer):
    def __init__(self, clv, Message, Headunit, UsbDevice):
        ...
        if debug.on(debug.Function):
            logfile.Write("clsTacxLegacyUsbTrainer.__init__()")
    ...
    def CurrentResistance2Power(self):
        if self.SpeedKmh == 0:
            ...
            rtn = (
                self.CurrentResistance
                * (self.SpeedKmh * self.SpeedKmh / 648 + self.SpeedKmh / 5411 + 0.1058)
                + 2.2 * self.SpeedKmh
            )
    ...
    def _ReceiveFromTrainer(self):
        if debug.on(debug.Function):
            logfile.Write("clsTacxLegacyUsbTrainer._ReceiveFromTrainer()")
        # -----------------------------------------------------------------------
        #  Read from trainer
        # -----------------------------------------------------------------------
        data = self.USB_Read()
...
class clsTacxNewUsbTrainer(clsTacxUsbTrainer):
    def __init__(self, clv, Message, Headunit, UsbDevice):
        ...
        while True:
            ...
            if not msgReceived and retry:
                if debug.on(debug.Any):
                    logfile.Write(
                        "Retry because no motor brake version message received"
                    )
                retry -= 1
            else:
                break
        ...
        # ---------------------------------------------------------------------------
        if debug.on(debug.Function):
            logfile.Write("clsTacxNewUsbTrainer.__init__() done")
    ...
    def CurrentResistance2Power(self):
        if self.MotorBrake:
            ...
            # -------------------------------------------------------------------
            self.CurrentPower = int(
                self.CurrentResistance / self.PowerResistanceFactor * self.WheelSpeed
            )
            ...
            # -------------------------------------------------------------------
            self.CurrentPower = int(
                self.Resistance2PowerMB(self.CurrentResistance, self.SpeedKmh)
            )
    ...
    def Resistance2PowerMB(self, Resistance, SpeedKmh):
        ...
        Speed = SpeedKmh / 3.6
        return Speed * (
            ScaleFactor * Resistance * Speed / (Speed + CriticalSpeed)
            + RollingResistance
        )
        ...
        if self.clv.Resistance:
            ...
            if self.WheelSpeed > 0:
                for i in range(0, len(self.currentR)):
                    if (
                        self.Resistance2PowerMB(self.currentR[i], self.SpeedKmh)
                        >= self.TargetPower
                    ):
                        break
                rtn = self.targetR[i]
    ...
    def _ReceiveFromTrainer(self):
        if debug.on(debug.Function):
            logfile.Write("clsTacxNewUsbTrainer._ReceiveFromTrainer()")
        # -----------------------------------------------------------------------
        # Read from trainer
        ...
        if len(data) < 40:
            ...
            if debug.on(debug.Function):
                logfile.Write(
                    "ReceiveFromTrainer() = hr=%s Buttons=%s Cadence=%s Speed=%s TargetRes=%s CurrentRes=%s CurrentPower=%s, pe=%s hdr=%s %s"
                    % (
                        self.HeartRate,
                        self.Buttons,
                        self.Cadence,
                        self.SpeedKmh,
                        self.TargetResistanceFT,
                        self.CurrentResistance,
                        self.CurrentPower,
                        self.PedalEcho,
                        hex(self.Header),
    ...
    def _ReceiveFromTrainer_MotorBrake(self):
        if debug.on(debug.Function):
            logfile.Write("clsTacxNewUsbTrainer._ReceiveFromTrainer_MotorBrake()...")
        # -----------------------------------------------------------------------
        # Define default
        ...
        if len(data) < 40:
            ...
            if self.MotorBrakeUnitSerial == 0:
                logfile.Console(
                    "Motor Brake Unit Firmware=%s Serial=%s MotorBrake=%s"
                    % (hex(self.MotorBrakeUnitFirmware), Serial, self.MotorBrake)
                )
            else:
                logfile.Console(
                    "Motor Brake Unit Firmware=%s Serial=%5s year=%s type=T19%s Version2=%s MotorBrake=%s"
                    % (
                        hex(self.MotorBrakeUnitFirmware),
                        Serial,
                        self.MotorBrakeUnitYear + 2000,
                        self.MotorBrakeUnitType,
                        self.Version2,
                        self.MotorBrake,
                    )
                )
        ...
        # ---------------------------------------------------------------------------
        if self.clv.Tacx_MotorBrake:
            self.MotorBrake = True
        if self.clv.Tacx_MagneticBrake:
            self.MotorBrake = False

...
class AntSCS(AntPlusInterface):
    ...
    def _broadcast_page(self, page_number: int, message_id=Id.BroadcastData):
        assert isinstance(self.data, sport.SpeedData)
        assert isinstance(self.data, sport.CadenceData)
        with self.data.lock:
            SpeedKmh = self.data.speed
            SpeedEchoTime = self.data.speed_event_time
            SpeedEchoCount = self.data.speed_revolution_count

            Cadence = self.data.cadence
            PedalEchoTime = self.data.cadence_event_time
            PedalEchoCount = self.data.cadence_revolution_count
    ...
    def _handle_broadcast_data(self, data_page_number: int, info: bytes):
        ...
        with self.data.lock:
            previous_values = [
                self.data.cadence_event_time,
                self.data.cadence_revolution_count,
                self.data.speed_event_time,
                self.data.speed_revolution_count,
            ]
            ...
            self.data.cadence_event_time = values[0]
            self.data.cadence_revolution_count = values[1]
            ...
            self.data.speed_event_time = values[2]
            self.data.speed_revolution_count = values[3]
        self.logger.info(

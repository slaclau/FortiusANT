...
device_types = {
    0x1902: {
        "name": 'T1902: Old "solid green" IMagic head unit (with or without loaded firmware)',
        "allowsfirmware": True,
    },
    0x1942: {
        "name": 'T1942: Old "solid blue" Fortius head unit (with firmware loaded)',
        "allowsfirmware": True,
    },
    0xE6BE: {
        "name": 'T1942: Old "solid blue" Fortius head unit (without firmware loaded)',
        "allowsfirmware": True,
    },
    0x1904: {
        "name": 'T1904: New "white green" IMagic head unit (white body with green or (sometimes) white ring - firmware inside)'
    },
    0x1932: {
        "name": 'T1932: New "white blue" VR head unit (white body with blue ring - firmware inside)'
    },
...
def device2name(device):
    return "DEVICE ID %04x:%04x on Bus %03d Adresse %03d" % (
        device.idVendor,
        device.idProduct,
        device.bus,
        device.address,
...
def writeEzusbVendor_RwInternal(device, addr, data):
    ...
    assert device.ctrl_transfer(
        usb.util.CTRL_TYPE_VENDOR | usb.util.CTRL_OUT | usb.util.CTRL_RECIPIENT_DEVICE,
        EZUSB_RW_INTERNAL,
        addr,
        0,
        data,
...
def loadHexFirmware(device, filename):
    ...
    with open(filename, "r") as file:
        ...
        for line in file:
            ...
            check = int(line[-3:], 16)
            data = bytearray(
                [int(line[i : i + 2], 16) for i in range(9, len(line) - 4, 2)]
            )
    ...
    with open(filename, "r") as file:
        for line in file:
            ...
            for i in range(0, bytecount):
                ...
                print(
                    ":{:02X}{:04X}{:02X}{:02X}{:02X}".format(
                        pos, address, rec_type, val, cks & 0xFF
                    )
                )
...
def main():
    ...
    parser.add_option(
        ...
        action="store_true",
        default=False,
        help="Force firmware download to unknown device.",
    )
    parser.add_option(
        "-a",
        "--ant",
        action="store_true",
        default=False,
        help="Allow (presumable emulated) Dynastream ANT Devices",
    )
    ...
    devices = [
        ...
        for device in usb.core.find(
            custom_match=lambda e: e.idVendor in vendors, find_all=True
        )
    ]
    ...
    for i in range(0, len(devices)):
        if args[i] == "-":
            ...
            device.idProduct in device_types
            and "allowsfirmware" in device_types[device.idProduct]
            and device_types[device.idProduct]["allowsfirmware"] == True
        ) or options.force:
            loadHexFirmware(devices[i], args[i])
        else:
            print(
                "Unknwon Device or device does not allow firmware download'. Use --force/-f Option to force download."
            )
            exit(1)

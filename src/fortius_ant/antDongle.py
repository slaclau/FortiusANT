...
def DeviceNumberBase(base):
    global DeviceNumber_EA, DeviceNumber_FE, DeviceNumber_HRM, DeviceNumber_VTX, DeviceNumber_VHU, DeviceNumber_SCS, DeviceNumber_PWR, DeviceNumber_CTRL
...
class clsAntDongle:
    ...
    def __GetDongle(self):
                ...
                for self.devAntDongle in devAntDongles:
                    if debug.on(debug.Function):
                        s = (
                            ...
                            % (
                                self.devAntDongle.manufacturer,
                                self.devAntDongle.product,
                                hex(self.devAntDongle.idVendor),
                                hex(self.devAntDongle.idProduct),
                            ...
                            while self.MessageQueueSize() > 0:
                                ...
                                if synch == 0xA4 and length == 0x01 and id == 0x6F:
                                    ...
                                    self.Message = self.Message.replace(
                                        "\0", ""
                                    )  # .manufacturer is NULL-terminated
        ...
        if not found_available_ant_stick:
            self.devAntDongle = None
        if debug.on(debug.Function):
            logfile.Write("GetDongle() returns: " + self.Message)
        ...
        if self.OK:  # If no dongle ==> no action at all
            ...
            for message in messages:
                ...
                if debug.on(debug.Performance):
                    logfile.Write(
                        "devAntDongle.write(0x01,%s) ..." % logfile.HexSpace(message)
                    )
                # -----------------------------------------------------------
                # Send the message
                ...
                # -----------------------------------------------------------
                try:
                    self.devAntDongle.write(
                        0x01, message
                    )  # input:   endpoint address, buffer, timeout
                    # returns:
                except Exception as e:
                    logfile.Console(
                        "AntDongle.Write exception (message lost): " + str(e)
                    )
    ...
    def __ReadAndRetry(self, timeout):
        ...
        # ----------------------------------------------------------------------
        if debug.on(debug.Performance):
            logfile.Write("devAntDongle.__ReadAndRetry(0x81,1000,%s) ..." % timeout)
        try:
            trv = []  # initialize because is processed even after exception
            trv = self.devAntDongle.read(
                0x81, 1000, timeout
            )  # input:  endpoint address, length, timeout
    ...
    def _Read(self, _drop, timeout=20):
        ...
        while self.OK:  # If no dongle ==> no action at all
            ...
            if len(trv) > 900:
                logfile.Console("Dongle.Read() too much data from .read()")
            start = 0
            while start < len(trv):
                error = False
                # -------------------------------------------------------
                ...
                if skip != start:
                    logfile.Console(
                        "Dongle.Read: %s characters skipped " % (skip - start)
                    )
                    start = skip
                # -------------------------------------------------------
                ...
                if start + 1 < len(trv):
                    ...
                    if start + length <= len(trv):
                        ...
                        if expected != checksum:
                            ...
                            logfile.Console(
                                "%s checksum=%s expected=%s data=%s"
                                % (
                                    error,
                                    logfile.HexSpace(checksum),
                                    logfile.HexSpace(expected),
                                    logfile.HexSpace(d),
                                )
                            )
                        else:
    ...
    # ---------------------------------------------------------------------------
    def Calibrate(self):
        if self.OK and debug.on(debug.Data1):
            logfile.Write("Calibrate()")
    ...
    def SlavePair_ChannelConfig(
        self, channel_pair, DeviceNumber=0, DeviceTypeID=0, TransmissionType=0
    ):
        ...
        if self.OK:
            ...
            if debug.on(debug.Data1):
                logfile.Write("SlavePair_ChannelConfig()")
        messages = [
            msg42_AssignChannel(
                channel_pair, ChannelType_BidirectionalReceive, NetworkNumber=0x00
            ),
        ...
        self.Write(
            messages
        )  # 2021-04-15 ", True, False"  removed because it's inconsistent
    ...
    def Trainer_ChannelConfig(self):
        if self.OK:
            ...
            if debug.on(debug.Data1):
                logfile.Write("Trainer_ChannelConfig()")
        messages = [
            msg42_AssignChannel(
                channel_FE, ChannelType_BidirectionalTransmit, NetworkNumber=0x00
            ),
            msg51_ChannelID(
                channel_FE, DeviceNumber_FE, DeviceTypeID_FE, TransmissionType_IC_GDP
            ),
    ...
    def SlaveTrainer_ChannelConfig(self, DeviceNumber):
        ...
        if self.OK:
            if self.ConfigMsg:
                logfile.Console(
                    "FortiusANT receives data from an ANT+ Controlled Fitness Equipent device (FE-C)"
                    + s
                )
            if debug.on(debug.Data1):
                logfile.Write("SlaveTrainer_ChannelConfig()")
        messages = [
            msg42_AssignChannel(
                channel_FE_s, ChannelType_BidirectionalReceive, NetworkNumber=0x00
            ),
            msg51_ChannelID(
                channel_FE_s, DeviceNumber, DeviceTypeID_FE, TransmissionType_Pairing
            ),
    ...
    def HRM_ChannelConfig(self):
        if self.OK:
            if self.ConfigMsg:
                logfile.Console(
                    "FortiusANT broadcasts data as an ANT+ Heart Rate Monitor (HRM), id=%s"
                    % DeviceNumber_HRM
                )
            if debug.on(debug.Data1):
                logfile.Write("HRM_ChannelConfig()")
        messages = [
            msg42_AssignChannel(
                channel_HRM, ChannelType_BidirectionalTransmit, NetworkNumber=0x00
            ),
            msg51_ChannelID(
                channel_HRM, DeviceNumber_HRM, DeviceTypeID_HRM, TransmissionType_IC
            ),
    ...
    def SlaveHRM_ChannelConfig(self, DeviceNumber):
        ...
        if self.OK:
            if self.ConfigMsg:
                logfile.Console(
                    "FortiusANT receives data from an ANT+ Heart Rate Monitor (HRM display)"
                    + s
                )
            if debug.on(debug.Data1):
                logfile.Write("SlaveHRM_ChannelConfig()")

        # 2021-12-03 Pairing must be done with TransmissionType=TransmissionType_Pairing (0)!!
        messages = [
            msg42_AssignChannel(
                channel_HRM_s, ChannelType_BidirectionalReceive, NetworkNumber=0x00
            ),
            msg51_ChannelID(
                channel_HRM_s, DeviceNumber, DeviceTypeID_HRM, TransmissionType_Pairing
            ),
    ...
    def PWR_ChannelConfig(self, DeviceNumber):
        if self.OK:
            if self.ConfigMsg:
                logfile.Console(
                    "FortiusANT broadcasts data as an ANT+ Bicycle Power Sensor (PWR), id=%s"
                    % DeviceNumber_PWR
                )
            if debug.on(debug.Data1):
                logfile.Write("PWR_ChannelConfig()")
        messages = [
            msg42_AssignChannel(
                channel_PWR, ChannelType_BidirectionalTransmit, NetworkNumber=0x00
            ),
            msg51_ChannelID(
                channel_PWR, DeviceNumber_PWR, DeviceTypeID_PWR, TransmissionType_IC
            ),
    ...
    def SCS_ChannelConfig(self, DeviceNumber):
        if self.OK:
            if self.ConfigMsg:
                logfile.Console(
                    "FortiusANT broadcasts data as an ANT+ Speed and Cadence Sensor (SCS), id=%s"
                    % DeviceNumber_SCS
                )
            if debug.on(debug.Data1):
                logfile.Write("SCS_ChannelConfig()")
        messages = [
            msg42_AssignChannel(
                channel_SCS, ChannelType_BidirectionalTransmit, NetworkNumber=0x00
            ),
            msg51_ChannelID(
                channel_SCS, DeviceNumber_SCS, DeviceTypeID_SCS, TransmissionType_IC
            ),
    ...
    def SlaveSCS_ChannelConfig(self, DeviceNumber):
        ...
        if self.OK:
            if self.ConfigMsg:
                logfile.Console(
                    "FortiusANT receives data from an ANT+ Speed and Cadence Sensor (SCS Display)"
                    + s
                )
            if debug.on(debug.Data1):
                logfile.Write("SlaveSCS_ChannelConfig()")
        messages = [
            msg42_AssignChannel(
                channel_SCS_s, ChannelType_BidirectionalReceive, NetworkNumber=0x00
            ),
            msg51_ChannelID(
    ...
    def CTRL_ChannelConfig(self, DeviceNumber):
        if self.OK:
            ...
            if self.ConfigMsg:
                logfile.Console(
                    "FortiusANT receives commands from an ANT+ Generic Remote Control"
                )
            if debug.on(debug.Data1):
                logfile.Write("CTRL_ChannelConfig()")
        messages = [
            msg42_AssignChannel(
                channel_CTRL, ChannelType_BidirectionalTransmit, NetworkNumber=0x00
            ),
            msg51_ChannelID(
                channel_CTRL, DeviceNumber, DeviceTypeID_CTRL, TransmissionType_IC
            ),
    ...
    def VTX_ChannelConfig(self):  # Pretend to be a Tacx Vortex
        if self.OK:
            if self.ConfigMsg:
                logfile.Console(
                    "FortiusANT broadcasts data as an ANT Tacx Vortex (VTX), id=%s"
                    % DeviceNumber_VTX
                )
            if debug.on(debug.Data1):
                logfile.Write("VTX_ChannelConfig()")
        messages = [
            msg42_AssignChannel(
                channel_VTX, ChannelType_BidirectionalTransmit, NetworkNumber=0x01
            ),
            msg51_ChannelID(
                channel_VTX, DeviceNumber_VTX, DeviceTypeID_VTX, TransmissionType_IC
            ),
    ...
    def SlaveVTX_ChannelConfig(self, DeviceNumber):  # Listen to a Tacx Vortex
        ...
        if self.OK:
            if self.ConfigMsg:
                logfile.Console(
                    "FortiusANT receives data from an ANT Tacx Vortex (VTX Controller)"
                    + s
                )
            if debug.on(debug.Data1):
                logfile.Write("SlaveVTX_ChannelConfig()")
        messages = [
            msg42_AssignChannel(
                channel_VTX_s, ChannelType_BidirectionalReceive, NetworkNumber=0x01
            ),
            msg51_ChannelID(
                channel_VTX_s, DeviceNumber, DeviceTypeID_VTX, TransmissionType_IC
            ),
    ...
    def SlaveGNS_ChannelConfig(self, DeviceNumber):  # Listen to a Tacx Genius
        ...
        if self.OK:
            if self.ConfigMsg:
                logfile.Console(
                    "FortiusANT receives data from an ANT Tacx Genius (GNS Brake)" + s
                )
            if debug.on(debug.Data1):
                logfile.Write("SlaveGNS_ChannelConfig()")
        messages = [
            msg42_AssignChannel(
                channel_GNS_s, ChannelType_BidirectionalReceive, NetworkNumber=0x01
            ),
            msg51_ChannelID(
                channel_GNS_s, DeviceNumber, DeviceTypeID_GNS, TransmissionType_IC
            ),
            msg45_ChannelRfFrequency(channel_GNS_s, RfFrequency_2460Mhz),
            msg43_ChannelPeriod(
                channel_GNS_s, ChannelPeriod=0x1000
    ...
    def SlaveBHU_ChannelConfig(self, DeviceNumber):  # Listen to a Tacx Genius
        ...
        if self.OK:
            if self.ConfigMsg:
                logfile.Console(
                    "FortiusANT receives data from an ANT Tacx Bushido head unit (BHU Controller)"
                    + s
                )
            if debug.on(debug.Data1):
                logfile.Write("SlaveBHU_ChannelConfig()")
        messages = [
            msg42_AssignChannel(
                channel_GNS_s, ChannelType_BidirectionalReceive, NetworkNumber=0x01
            ),
            msg51_ChannelID(
                channel_GNS_s, DeviceNumber, DeviceTypeID_BHU, TransmissionType_IC
            ),
            msg45_ChannelRfFrequency(channel_GNS_s, RfFrequency_2460Mhz),
            msg43_ChannelPeriod(channel_GNS_s, ChannelPeriod=0x1000),
            msg60_ChannelTransmitPower(channel_GNS_s, TransmitPower_0dBm),
            msg44_ChannelSearchTimeout(
                channel_GNS_s, 255
            ),  # keep searching indefinitely
            msg4B_OpenChannel(channel_GNS_s),
            msg4D_RequestMessage(channel_GNS_s, msgID_ChannelID),
    ...
    def SlaveVHU_ChannelConfig(self, DeviceNumber):  # Listen to a Tacx Vortex Headunit
        ...
        if self.OK:
            if self.ConfigMsg:
                logfile.Console(
                    "FortiusANT receives data from an ANT Tacx Vortex Headunit (VHU Controller)"
                    + s
                )
            if debug.on(debug.Data1):
                logfile.Write("SlaveVHU_ChannelConfig()")
        messages = [
            msg42_AssignChannel(
                channel_VHU_s, ChannelType_BidirectionalReceive, NetworkNumber=0x01
            ),
            msg51_ChannelID(
                channel_VHU_s, DeviceNumber, DeviceTypeID_VHU, TransmissionType_IC
            ),
...
def DongleDebugMessage(text, d):
    if debug.on(debug.Data1):
        ...
        # -----------------------------------------------------------------------
        if id == msgID_ANTversion:
            id_ = "ANT version"

        elif id == msgID_BroadcastData:
            id_ = "Broadcast Data"
        elif id == msgID_AcknowledgedData:
            id_ = "Acknowledged Data"

        elif id == msgID_ChannelResponse:
            id_ = "Channel Response"
        elif id == msgID_Capabilities:
            id_ = "Capabilities"
        elif id == msgID_UnassignChannel:
            id_ = "Unassign Channel"
        elif id == msgID_AssignChannel:
            id_ = "Assign Channel"
        elif id == msgID_ChannelPeriod:
            id_ = "Channel Period"
        elif id == msgID_ChannelSearchTimeout:
            id_ = "Channel Search Timeout"
        elif id == msgID_ChannelRfFrequency:
            id_ = "Channel RfFrequency"
        elif id == msgID_SetNetworkKey:
            id_ = "Set NetworkKey"
        elif id == msgID_ResetSystem:
            id_ = "Reset System"
        elif id == msgID_OpenChannel:
            id_ = "Open Channel"
        elif id == msgID_RequestMessage:
            id_ = "Request Message"
        elif id == msgID_ChannelID:
            id_ = "Channel ID"
        elif id == msgID_ChannelTransmitPower:
            id_ = "Channel TransmitPower"
        elif id == msgID_StartUp:
            id_ = "Start up"
        elif id == msgID_RF_EVENT:
            id_ = "RF event"  # D00000652..._Rev_5.1.pdf 9.5.6.1 Channel response
        else:
            id_ = "??"
        ...
        if id == msgID_ChannelResponse or id == msgID_RequestMessage:
            ...
            extra = info.decode("utf-8").replace(
                "\0", ""
            )  # ANTversion in string format
            info_ = ""
            ...
            if p < 0:
                ...
                p_ = "Default data page"  # D00000693_-_ANT+_Device_Profile_-_Heart_Rate_Rev_2.1
            ...
            #    e.g. dependant on Channel
                ...
                p_ = "HRM Cumulative Operating Time"
            elif p & 0x7F == 2:
                p_ = "HRM Manufacturer info"
            elif p & 0x7F == 3:
                p_ = "HRM Product information"
            elif p & 0x7F == 4:
                p_ = "HRM Previous Heart beat"
            elif p & 0x7F == 5:
                p_ = "HRM Swim interval summary"
            elif p & 0x7F == 6:
                p_ = "HRM Capabilities"
            elif p == 16:
                p_ = "Main data page"
                ...
                p_ = "User Configuration"
            elif p == 70:
                p_ = "Request Datapage"
            elif p == 76:
                p_ = "Mode settings page"
            elif p == 80:
                p_ = "Manufacturer Info"
            elif p == 81:
                p_ = "Product Information"
            elif p == 82:
                p_ = "Battery Status"
            #           elif p        == 89: p_ = 'Add channel ID to list ???'
            elif p == 172:
                p_ = "Tacx Request information/Set mode"
            elif p == 173:
                p_ = "Tacx Device information"
            elif p == 220:
                p_ = "Tacx Brake control"
            elif p == 221:
                p_ = "Tacx Data update"
            else:
                p_ = "??"
        ...
        if extra != "":
            ...
            if Channel == -1:
                extra = ""  # No Channel, do not show
            else:
                extra = " (ch=%s%s)" % (
                    Channel,
                    p_,
                )  # Channel, show it with optional pageinfo

        # -----------------------------------------------------------------------
        # Write to logfile
        # -----------------------------------------------------------------------
        logfile.Write(
            "%s synch=%s, len=%2s, id=%s %-21s, check=%4s, info=%s%s"
...
def msgPage00_TacxVortexDataSpeed(Channel, Power, Speed, Cadence):
    ...
    fDataPageNumber = (
        sc.unsigned_char
    )  # payload[0]        First byte of the ANT+ datapage
    ...
    info = struct.pack(
        format, Channel, DataPageNumber, int(Power), int(Speed), int(Cadence)
    )
...
def msgUnpage01_TacxVortexDataSerial(info):
    ...
    format = (
        sc.big_endian + fChannel + fDataPageNumber + fS1 + fS2 + fS3 + fSerial + fAlarm
    )
    tuple = struct.unpack(format, info)
...
def msgPage16_TacxVortexSetCalibrationValue(Channel, VortexID, CalibrationValue):
    ...
    info = struct.pack(
        format, Channel, DataPageNumber, VortexID, 0, 0x7F, CalibrationValue
    )
...
def msgPage16_TacxVortexSetPower(Channel, VortexID, Power):
    ...
    info = struct.pack(
        format, Channel, DataPageNumber, int(VortexID), 0xAA, 0, 0, int(Power)
    )
...
def msgPage220_01_TacxGeniusSetTarget(Channel, Mode, Target, Weight):
    ...
    info = struct.pack(
        format, Channel, DataPageNumber, SubPageNumber, Mode, Target, Weight
    )
...
def msgPage220_02_TacxGeniusWindResistance(Channel, WindResistance, WindSpeed):
    ...
    fWindResistance = (
        sc.unsigned_short
    )  # 0.5 * wind resistance cofficient (kg/m) * 1000
    ...
    info = struct.pack(
        format, Channel, DataPageNumber, SubPageNumber, WindResistance, WindSpeed
    )
...
def msgPage220_04_TacxGeniusCalibration(Channel, Action):
    ...
    format = (
        sc.big_endian + fChannel + fDataPageNumber + fSubPageNumber + fAction + fPadding
    )
...
# ------------------------------------------------------------------------------
def msgPage25_TrainerData(
    Channel, EventCounter, Cadence, AccumulatedPower, CurrentPower
):
    DataPageNumber = 25
...
def msgUnpage70_RequestDataPage(info):
    ...
    return (
        tuple[nSlaveSerialNumber],
        tuple[nDescriptorByte1],
        tuple[nDescriptorByte2],
        AckRequired,
        NrTimes,
        tuple[nRequestedPageNumber],
        tuple[nCommandType],
    )
...
# D00001198_-_ANT+_Common_Data_Pages_Rev_3.1.pdf
# ------------------------------------------------------------------------------
def msgPage73_GenericCommand(
    Channel, SlaveSerialNumber, SlaveManufacturerID, SequencNr, CommandNr
):
    DataPageNumber = 73
...
# ------------------------------------------------------------------------------
def msgPage80_ManufacturerInfo(
    Channel, Reserved1, Reserved2, HWrevision, ManufacturerID, ModelNumber
):
    DataPageNumber = 80
...
# ------------------------------------------------------------------------------
def msgPage81_ProductInformation(
    Channel, Reserved1, SWrevisionSupp, SWrevisionMain, SerialNumber
):
    DataPageNumber = 81
...
def msgPage82_BatteryStatus(Channel):
    ...
    info = struct.pack(
        format, Channel, DataPageNumber, 0xFF, 0x00, 0, 0, 0, 0, 0x0F | 0x10 | 0x00
    )
...
def msgPage_Hrm(
    ...
    HeartBeatEventTime = int(
        min(0xFFFF, HeartBeatEventTime * 1000 / 1024)
    )  # Convert seconds into 1024seconds (since ever)

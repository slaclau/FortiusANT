...
class clsRadarGraph:
    ...
    def PedalStrokeAnalysis(self, info, Cadence):
        ...
        data = []
        logfile.Write(
            "PedalStrokeAnalysis - Cadence=%3s info=%3s StartTime=%s"
    ...
    def OnPaint(self, dc):
        ...
        dc.SetBrush(
            wx.Brush(wx.Colour(139, 193, 227))
        ...
        dc.SetFont(
            wx.Font(int(12 * self.wh / 300), wx.SWISS, wx.NORMAL, wx.NORMAL)
        )  # pylint: disable=maybe-no-member
        tw, th = dc.GetTextExtent(self.title)
        dc.DrawText(self.title, int(self.cx - tw / 2), int(self.cy - th))

        dc.SetFont(
            wx.Font(int(8 * self.wh / 300), wx.SWISS, wx.NORMAL, wx.NORMAL)
        )  # pylint: disable=maybe-no-member
        tw, th = dc.GetTextExtent("100%")
        dc.DrawText(
            "%sW" % int(0.25 * self.maxval),
            int(self.cx + 0.25 * self.radius100 - tw),
            self.cy,
        )
        dc.DrawText(
            "%sW" % int(0.50 * self.maxval),
            int(self.cx + 0.50 * self.radius100 - tw),
            self.cy,
        )
        dc.DrawText(
            "%sW" % int(0.75 * self.maxval),
            int(self.cx + 0.75 * self.radius100 - tw),
            self.cy,
        )
        dc.DrawText(
            "%sW" % int(1.00 * self.maxval),
            int(self.cx + 1.00 * self.radius100 - tw),
            self.cy,
        )
        ...
        dc.DrawLine(
            int(self.cx - self.radius100 * 1.1),
            int(self.cy),
            int(self.cx + self.radius100 * 1.1),
            int(self.cy),
        )
        dc.DrawLine(
            int(self.cx),
            int(self.cy - self.radius100 * 1.1),
            int(self.cx),
            int(self.cy + self.radius100 * 1.1),
        )
...
if __name__ == "__main__":
    ...
    class TestFrame(wx.Frame):  # pylint: disable=maybe-no-member
        ...
        def __init__(self):
            # pylint: disable=maybe-no-member
            wx.Frame.__init__(
                self, None, title="Double Buffered Drawing", size=(680, 680)
            )
            # ------------------------------------------------------------------

...
class clsSteering:
    ...
    def __init__(
        self, InitialCalLeft, InitialCalRight, CalStabilityLimit=15, DeadZone=0.0
    ):
    ...
    def Update(self, RawAngle):
        ...
        if debug.on(debug.Function):
            logfile.Write(
                "Steering: CalLeft=%d, CalMid=%d, CalRight=%d"
                % (self.CalLeft, self.CalMid, self.CalRight)
            )
...
class clsBlackTrack:
    def __init__(self, AntDevice):
        ...
        self.Steering = clsSteering(
            InitialCalLeft=-300, InitialCalRight=300, DeadZone=2.0
        )
        ...
        if channel == self._Channel:
            ...
            if msgId == ant.msgID_BroadcastData:
                ...
                if dataPageNumber == 0:
                    ...
                    if debug.on(debug.Function):
                        logfile.Write(
                            "Tacx BlackTrack Page=%d (IN)  RawAngle=%d"

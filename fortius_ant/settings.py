    ...
    try:
        ...
        try:
            ...
            for p in data:
                ...
                if p == json_Ant:
                    for q in data[json_Ant]:
                        if q == json_antDeviceID:
                            args.antDeviceID = w[q]
                        #'CtrlCommand':                   args.CtrlCommand
                        #'scs':                           args.scs                  = p[q]
                        else:
                            logfile.Console(
                                "Json file contains unknown parameter %s %s" % (p, q)
                            )

                elif p == json_Bluetooth:
                    for q in data[json_Bluetooth]:
                        if q == json_ble:
                            args.ble = w[q]  # For compatibility
                        elif q == json_nodejs:
                            args.ble = w[q]
                        elif q == json_bless:
                            args.bless = w[q]
                    ...
                    for q in data[json_General]:
                        if q == json_autostart:
                            ...
                            args.debug = w[q]
                        elif q == json_exportTCX:
                            args.exportTCX = w[q]
                        elif q == json_gui:
                            args.gui = w[q]
                        elif q == json_homeTrainer:
                    ...
                    for q in data[json_Simulation]:
                        if q == json_manual:
                            args.manual = w[q]
                        elif q == json_manualGrade:
                            args.manualGrade = w[q]
                        elif q == json_Resistance:
                            args.Resistance = w[q]
                        elif q == json_simulate:
                            args.simulate = w[q]
                        else:
                            logfile.Console(
                                "Json file contains unknown parameter %s %s" % (p, q)
                    ...
                    for q in data[json_Trainer]:
                        if q == json_PedalStrokeAnalysis:
                            args.PedalStrokeAnalysis = w[q]
                        elif q == json_CalibrateRR:
                            args.CalibrateRR = w[q]
                        elif q == json_antDeviceID:
                            args.antDeviceID = w[q]
                        elif q == json_GradeFactor:
                            G += 1
                        elif q == json_GradeFactorDH:
                            G += 1
                        elif q == json_calibrate:
                            args.calibrate = w[q]
                        elif q == json_factor:
                            args.factor = w[q]
                        elif q == json_PowerMode:
                            args.PowerMode = w[q]
                        elif q == json_TacxType:
                            args.TacxType = w[q]
                        elif q == json_Transmission:
                            args.Transmission = w[q]
                        elif q == json_Runoff:
                            for r in w[q]:
                                if r == json_RunoffMaxSpeed:
                                    R += 1
                                elif r == json_RunoffDip:
                                    R += 1
                                elif r == json_RunoffMinSpeed:
                                    R += 1
                                elif r == json_RunoffTime:
                                    R += 1
                                elif r == json_RunoffPower:
                                    R += 1
                                else:
                                    logfile.Console(
                                        "Json file contains unknown parameter %s %s %s"
                                        % (p, q, r)
                                    )
                        else:
                            logfile.Console(
            ...
            if ActualVersion != Version:
                logfile.Console(
                    "JSON file has old format (%s=%s expected %s), not all parameters may be recognized."
            ...
            if R == 5:
                ...
                if (
                    RunoffMaxSpeed == "40"
                    and RunoffDip == "2"
                    and RunoffMinSpeed == "1"
                    and RunoffTime == "7.0"
                    and RunoffPower == "100"
                ):
                    args.Runoff = False
                else:
                    args.Runoff = "%s/%s/%s/%s/%s" % (
                        RunoffMaxSpeed,
                        RunoffDip,
                        RunoffMinSpeed,
                        RunoffTime,
...
if constants.UseGui:
    ...
    def OpenDialog(app, parent=None, pclv=None):
        ...
        if parent:
            dlg.SetPosition(
                (
                    parent.Position[0] + parent.Speed.Position[0],
        ...
        if debug.on(debug.Function):
            logfile.Write(
                "settings.OpenDialog returns restart=%s debug=%s"
                % (RestartApplication, clv.debug)
            )
    ...
    def RightOf(control, Xoffset=5, Yoffset=2):
        return (
            control.Position[0] + control.Size[0] + Xoffset,
            control.Position[1] + Yoffset,
        )
    ...
    class dlgFortiusAntSettings(wx.Dialog):
        ...
        def __init__(self, parent):
            ...
            GroupLabelFont = wx.Font(
                -1, family=wx.DEFAULT, style=wx.NORMAL, weight=wx.BOLD
            )
            ...
            self.cb_restart.SetPosition(
                (self.cb_restart.Position[0] + moveRight, self.cb_restart.Position[1])
            )
            self.cb_saveJson.SetPosition(
                (self.cb_saveJson.Position[0] + moveRight, self.cb_saveJson.Position[1])
            )
            self.btnOK.SetPosition(
                (self.btnOK.Position[0] + moveRight, self.btnOK.Position[1])
            )
            self.btnCancel.SetPosition(
                (self.btnCancel.Position[0] + moveRight, self.btnCancel.Position[1])
            )
            self.btnHelp.SetPosition(
                (self.btnHelp.Position[0] + moveRight, self.btnHelp.Position[1])
            )
            ...
            if clv:
                ...
                if clv.antDeviceID:
                    self.txt_D.SetValue(str(clv.antDeviceID))
                if clv.hrm:
                    self.txt_H.SetValue(str(clv.hrm))
                if clv.CalibrateRR:
                ...
                if not clv.args.debug:
                    self.txt_d.SetValue(
                        ""
                    )  # make difference between 0 and not-specified
        ...
        def EVT_CHECKBOX_cb_bb(self, event):
            self.cb_restart.SetValue(True)
            if self.cb_bb.GetValue():
                self.cb_b.SetValue(False)
        ...
        def EVT_CHECKBOX_cb_d(self, event):
            ...
            if clv.debug == 0 and i != 0:
                self.cb_restart.SetValue(True)

        def EVT_KILL_FOCUS_txt_d(self, event=None):
            if event:
                EVT_KILL_FOCUS_int_range(self.txt_d, event, 0, 127, "")
            try:
        ...
        def EVT_CHECKBOX_cb_g(self, event):
            ...
            if (
                self.cb_g.GetValue() == False
                and wx.MessageBox(
                    msg, TitleText, wx.YES_NO | wx.NO_DEFAULT | wx.ICON_QUESTION
                )
                == wx.NO
            ):
        ...
        def EVT_CHECKBOX_cb_restart(self, event):
            ...
            if (
                self.cb_restart.GetValue() == False
                and wx.MessageBox(
                    msg,
                    TitleText,

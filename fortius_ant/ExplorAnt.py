...
class clsDeviceID(object):
    def __init__(
        self, Channel, DeviceType, DeviceNumber, DeviceTypeID, TransmissionType
    ):
...
# ------------------------------------------------------------------------------
    ...
    if AntDongle.OK and not clv.SimulateTrainer:
        ...
        for i in range(0, NrDevicesToPair):
            ...
            if i == ant.channel_VTX_s:
                ...
                if i % 1 == 1:
                    DeviceTypeID &= 0x80
                AntDongle.SlavePair_ChannelConfig(
                ...
                while AntDongle.MessageQueueSize() > 0:
                    ...
                    # ---------------------------------------------------------------
                        ...
                        if DeviceNumber == 0:  # No device paired, request again
                            ...
                            d = deviceID
                            logfile.Write(
                                "ExplorANT: %3s discovered on channel=%s, number=%5s typeID=%3s TrType=%3s"
                                % (
                                    d.DeviceType,
                                    d.Channel,
                                    d.DeviceNumber,
                            ...
                            if not deviceID in deviceIDs:
                                ...
                                info = ant.msgPage70_RequestDataPage(
                                    Channel, ant.DeviceNumber_EA, 255, 255, 1, 70, 0
                                )
                        ...
                        logfile.Console(
                            "Ignore message id=%s ch=%s page=%s"
        ...
        for d in deviceIDs:
            logfile.Console(
                " %3s discovered on channel=%s, number=%5s typeID=%3s TrType=%3s"
                % (
                    d.DeviceType,
                    d.Channel,
                    d.DeviceNumber,
            ...
            try:
                ...
                while RunningSwitch == True and not AntDongle.DongleReconnected:
                            ...
                            if (
                                ...
                                messages.append(
                                    ant.ComposeMessage(
                                        ...
                                        ant.msgPage03_TacxVortexDataCalibration(
                                            ant.channel_VTX, 0, ant.DeviceNumber_VTX
                                        ),
                                    )
                                )
                    ...
                    if AntDongle.MessageQueueSize() == 0 and VortexData:
                        VortexData = False
                        messages = [
                            "a4 09 4e 07 00 00 03 60 08 00 8a 00 05",
                        ...
                        # -----------------------------------------------------------
                                ...
                                if DataPageNumber & 0x7F in (0, 1, 2, 3, 4, 5, 6, 7, 89):
                                    ...
                                    if DataPageNumber & 0x7F == 2:
                                        if HRM_page2_done == False:
                                            ...
                                            logfile.Console(
                                                "HRM page=%s ManufacturerID=%s SerialNumber=%s"
                                                % (
                                                    DataPageNumber,
                                                    HRM_ManufacturerID,
                                                    HRM_SerialNumber,
                                    ...
                                    if DataPageNumber & 0x7F == 3:
                                        if HRM_page3_done == False:
                                            ...
                                            HRM_Model = Spec3
                                            logfile.Console(
                                                "HRM page=%s HWrevision=%s, SWversion=%s Model=%s"
                                                % (
                                                    DataPageNumber,
                                                    HRM_HWrevision,
                                                    HRM_SWversion,
                            ...
                            # -------------------------------------------------------
                                ...
                                # ---------------------------------------------------
                                    ...
                                    if FE_page80_done == False:
                                        ...
                                        logfile.Console(
                                            "FE Page=%s HWrevision=%s ManufacturerID=%s Model=%s"
                                            % (
                                                DataPageNumber,
                                                FE_HWrevision,
                                                FE_ManufacturerID,
                                ...
                                # ---------------------------------------------------
                                    ...
                                    if FE_page81_done == False:
                                        ...
                                        logfile.Console(
                                            "FE Page=%s SWrevision=%s.%s Serial#=%s"
                                            % (
                                                DataPageNumber,
                                                FE_SWrevisionMain,
                                                FE_SWrevisionSupp,
                            ...
                            if Channel == ant.channel_SCS_s:
                                ...
                                try:
                                    ...
                                    if EventTime != pEventTime:
                                        ...
                                        print(
                                            "EventTime=%5s (%5s) CadenceRevolutionCount=%5s (%5s) Cadence=%3s EventTime=%5s SpeedRevolutionCount=%5s Speed=%4.1f"
                                            % (
                                                EventTime,
                                                EventTime - pEventTime,
                                                CadenceRevolutionCount,
                            ...
                            # -------------------------------------------------------
                                ...
                                # ---------------------------------------------------
                                    ...
                                    logfile.Console(
                                        "i-Vortex Page=%s S1=%s S2=%s Serial=%s Alarm=%s"
                                        % (
                                            DataPageNumber,
                                            VTX_S1,
                                            VTX_S2,
                    ...
                    if listenCount == 4:
                        ...
                        if clv.SimulateTrainer:
                            logfile.Console(
                                (
                                    "Simulate Cadence=%3s Power=%3s Speed=%4.1f hr=%3s "
                    ...
                    if (
                        ...
                        info = ant.msgPage16_TacxVortexSetPower(
                            ant.channel_VTX_s, VTX_VortexID, Power
                        )
    ...
    while AntDongle.OK:
        ...
        if (
            ...
            messages = []
            if clv.hrm > 0:
                messages.append(ant.msg41_UnassignChannel(ant.channel_HRM_s))
            if clv.fe > 0:
                messages.append(ant.msg41_UnassignChannel(ant.channel_FE_s))
            AntDongle.Write(messages)
            # ---------------------------------------------------------------
            # Release dongle
            # -----------------------------------------------------------------------
    ...
    logfile.Console("--------------------")
    essages.append(ant.msg41_UnassignChannel(ant.channel_HRM_s))
            if clv.fe > 0:
                messages.append(ant.msg41_UnassignChannel(ant.channel_FE_s))
            AntDongle.Write(messages)
            # ---------------------------------------------------------------

...
class CommandLineVariables(object):
    ...
    gui = False
    hrm = (
        None  # introduced 2020-02-09; None=not specified, numeric=HRM device, -1=no HRM
    )
    homeTrainer = False
    ...
    def __init__(self):
        ...
        parser = argparse.ArgumentParser(
            description="FortiusAnt exchanges data between USB or ANT+ Tacx trainer with an ANT+ or BLE Cycling Training Program"
        )
        parser.add_argument(
            "-a",
            dest="autostart",
        ...
        self.exportTCX = (
            self.args.exportTCX or self.homeTrainer or self.manual or self.manualGrade
        )
        ...
        if i > 1:
            logfile.Console(
                "Only one of -e, -m and -M may be specified; manual power selected"
            )
        ...
        if (
            self.homeTrainer or self.manual or self.manualGrade
        ) and self.SimulateTrainer:
            logfile.Console(
        ...
        if OnRaspberry and self.args.OutputDisplay:
            ...
            try:
                ...
                if len(s) >= 1:
                    self.OutputDisplay = s[0]
                if len(s) >= 2:
                    self.OutputDisplayR = int(s[1])  # Rotation
        ...
        if OnRaspberry and self.args.gpioLayout:
            ...
            try:
                ...
                if len(s) >= 1:
                    self.rpiButton = int(s[0])
                if len(s) >= 2:
                    self.rpiTacx = int(s[1])
                if len(s) >= 3:
                    self.rpiShutdown = int(s[2])
                if len(s) >= 4:
                    self.rpiCadence = int(s[3])
                if len(s) >= 5:
                    self.rpiBLE = int(s[4])
                if len(s) >= 6:
                    self.rpiANT = int(s[5])
                ...
                assert self.rpiShutdown not in (
                    self.rpiButton,
                    self.rpiTacx,
                    self.rpiCadence,
                    self.rpiBLE,
        ...
        if self.args.debug != None:
            try:
                ...
                if "A" in self.args.debug:
                    self.debug |= debug.Application
                if "f" in self.args.debug:
                    self.debug |= debug.Function
                if "a" in self.args.debug:
                    self.debug |= debug.Data1
                if "u" in self.args.debug:
                    self.debug |= debug.Data2
                if "m" in self.args.debug:
                    self.debug |= debug.MultiProcessing
                if "j" in self.args.debug:
                    self.debug |= debug.LogfileJson
                if "b" in self.args.debug:
                    self.debug |= debug.Ble
                if "p" in self.args.debug:
                    self.debug |= debug.Performance

                if "D" in self.args.debug:
                    self.debug |= debug.logging_DEBUG
                if "I" in self.args.debug:
                    self.debug |= debug.logging_INFO
                if "W" in self.args.debug:
                    self.debug |= debug.logging_WARNING
                if "E" in self.args.debug:
                    self.debug |= debug.logging_ERROR
                if "C" in self.args.debug:
                    self.debug |= debug.logging_CRITICAL
        ...
        if self.args.hrm != None:
            try:
                ...
                logfile.Console(
                    "Command line error; -H incorrect HRM=%s" % self.args.hrm
                )
        ...
        if self.args.TacxType:
            ...
            if "Vortex" in self.TacxType:
                ...
                logfile.Console(
                    "Command line error; -t incorrect value=%s" % self.args.TacxType
                )
        ...
        if UseBluetooth:
            ...
            if self.Steering == "wired" and (
                self.Tacx_Genius or self.Tacx_Bushido or self.Tacx_Vortex
            ):
                logfile.Console(
                    "Wired steering (-S wired) not available on with ANT-trainer (-t %s); -S wired ignored."
                    % self.TacxType
                )
                self.Steering = None
        ...
        # -----------------------------------------------------------------------
        if self.PedalStrokeAnalysis and (
            not self.args.gui
            or self.Tacx_Vortex
            or self.Tacx_Genius
    ...
    def print(self):
        try:
            ...
            if self.bless:
                logfile.Console("-bb")
            if v or self.args.DeviceNumberBase != None:
                logfile.Console("-B %s" % self.DeviceNumberBase)
            if v or self.args.CalibrateRR != None:
            ...
            if v or self.args.debug != None:
                logfile.Console(
                    "-d %s (%s, %s)" % (self.args.debug, self.debug, bin(self.debug))
                )
            if v or self.args.antDeviceID != None:
                logfile.Console("-D %s" % self.antDeviceID)
            if v or self.args.GradeAdjust:
                if self.GradeAdjust == 1:
                    logfile.Console(
                        "-G defines Grade = antGrade * %s" % (self.GradeFactor)
                    )
                if self.GradeAdjust == 2:
                    logfile.Console(
                        "-G defines Grade = antGrade * %s [* %s (downhill)]"
                        % (self.GradeFactor, self.GradeFactorDH)
                    )
                if self.GradeAdjust == 3:
                    logfile.Console(
                        "-G defines Grade = (antGrade - %s) * %s [* %s (downhill)]"
            ...
            if self.homeTrainer:
                logfile.Console("-e")
            if v or self.args.hrm != None:
                logfile.Console("-H %s" % self.hrm)
            if self.StatusLeds:
                logfile.Console("-l")
            ...
            if not self.args.calibrate:
                logfile.Console("-n")
            if v or self.args.factor != None:
                logfile.Console("-p %s" % self.PowerFactor)
            if v or self.args.OutputDisplay:
                logfile.Console("-O %s" % self.OutputDisplay)
            if self.args.PowerMode:
                logfile.Console("-P")
            if self.args.Resistance:
                logfile.Console("-r")
            if v or self.args.Runoff:
                logfile.Console(
                    "-R defines Runoff: maxSpeed=%s dip=%s minSpeed=%s targetTime=%s power=%s"
                    % (
                        self.RunoffMaxSpeed,
                        self.RunoffDip,
                        self.RunoffMinSpeed,
            ...
            if self.args.Steering is not None:
                logfile.Console("-S %s" % self.args.Steering)

    ...
    if UseBluetooth:
        ...
        def Open(self):
            ...
            if debug.on(debug.Ble):
                logfile.Write("BleInterface.Open() ...")

            if self.interface:
                if debug.on(debug.Ble):
                    logfile.Write("... already open")
            else:
                # -----------------------------------------------------------
                ...
                dirname = os.path.dirname(
                    dirname
                )  # Remove /filename.py or /filename.exe
        ...
        def Read(self):
            if debug.on(debug.Ble):
                logfile.Write("BleInterface.Read() ...")
            rtn = False
            self.jsondata = None
            if self.interface:
                # -----------------------------------------------------------
        ...
        if UseBluetooth:
            ...
            if self.OK and self.Write(data):
                if self.Read():
                    ...
                    try:
                        self.WindResistance = self.jsondata[
                    ...
                    try:
                        self.RollingResistance = self.jsondata[
